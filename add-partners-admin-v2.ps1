# ============================================================
# Add Partners Admin script v2 (Windows PowerShell 5.1 safe)
#
# Corrected version: all em-dashes, box-drawing divider comments,
# and ellipsis characters replaced with plain ASCII equivalents
# (-, -, ...) to eliminate the encoding corruption seen when
# these files were first written.
#
# Writes 8 files implementing full admin CRUD for Partners,
# mirroring the existing Programs/Opportunities admin pattern.
#
# IMPORTANT: before running this, run this SQL in the Supabase
# SQL Editor if you haven't already:
#   alter type audit_entity add value if not exists 'partner';
#
# Run this from your project ROOT (where package.json lives):
#   .\add-partners-admin-v2.ps1
# ============================================================
...
if ($check_actions -match [regex]::Escape("createPartner")) { Write-Host "OK: src\actions\partners.ts contains expected content" -ForegroundColor Green } else { Write-Host "PROBLEM: src\actions\partners.ts missing expected content" -ForegroundColor Red }
if ($check_actions -match "[\u00C0-\u00FF]{2,}") { Write-Host "WARNING: src\actions\partners.ts may contain garbled encoding" -ForegroundColor Yellow }
$check_table = Get-Content -LiteralPath "src\components\admin\PartnersTable.tsx" -Raw
if ($check_table -match [regex]::Escape("PartnersTable")) { Write-Host "OK: src\components\admin\PartnersTable.tsx contains expected content" -ForegroundColor Green } else { Write-Host "PROBLEM: src\components\admin\PartnersTable.tsx missing expected content" -ForegroundColor Red }
if ($check_table -match "[\u00C0-\u00FF]{2,}") { Write-Host "WARNING: src\components\admin\PartnersTable.tsx may contain garbled encoding" -ForegroundColor Yellow }
$check_form = Get-Content -LiteralPath "src\components\admin\PartnerForm.tsx" -Raw
if ($check_form -match [regex]::Escape("PartnerForm")) { Write-Host "OK: src\components\admin\PartnerForm.tsx contains expected content" -ForegroundColor Green } else { Write-Host "PROBLEM: src\components\admin\PartnerForm.tsx missing expected content" -ForegroundColor Red }
if ($check_form -match "[\u00C0-\u00FF]{2,}") { Write-Host "WARNING: src\components\admin\PartnerForm.tsx may contain garbled encoding" -ForegroundColor Yellow }
$check_list_page = Get-Content -LiteralPath "src\app\admin\(dashboard)\partners\page.tsx" -Raw
if ($check_list_page -match [regex]::Escape("AdminPartnersPage")) { Write-Host "OK: src\app\admin\(dashboard)\partners\page.tsx contains expected content" -ForegroundColor Green } else { Write-Host "PROBLEM: src\app\admin\(dashboard)\partners\page.tsx missing expected content" -ForegroundColor Red }
if ($check_list_page -match "[\u00C0-\u00FF]{2,}") { Write-Host "WARNING: src\app\admin\(dashboard)\partners\page.tsx may contain garbled encoding" -ForegroundColor Yellow }
$check_new_page = Get-Content -LiteralPath "src\app\admin\(dashboard)\partners\new\page.tsx" -Raw
if ($check_new_page -match [regex]::Escape("NewPartnerPage")) { Write-Host "OK: src\app\admin\(dashboard)\partners\new\page.tsx contains expected content" -ForegroundColor Green } else { Write-Host "PROBLEM: src\app\admin\(dashboard)\partners\new\page.tsx missing expected content" -ForegroundColor Red }
if ($check_new_page -match "[\u00C0-\u00FF]{2,}") { Write-Host "WARNING: src\app\admin\(dashboard)\partners\new\page.tsx may contain garbled encoding" -ForegroundColor Yellow }
$check_edit_page = Get-Content -LiteralPath "src\app\admin\(dashboard)\partners\[slug]\page.tsx" -Raw
if ($check_edit_page -match [regex]::Escape("EditPartnerPage")) { Write-Host "OK: src\app\admin\(dashboard)\partners\[slug]\page.tsx contains expected content" -ForegroundColor Green } else { Write-Host "PROBLEM: src\app\admin\(dashboard)\partners\[slug]\page.tsx missing expected content" -ForegroundColor Red }
if ($check_edit_page -match "[\u00C0-\u00FF]{2,}") { Write-Host "WARNING: src\app\admin\(dashboard)\partners\[slug]\page.tsx may contain garbled encoding" -ForegroundColor Yellow }
$check_loading_page = Get-Content -LiteralPath "src\app\admin\(dashboard)\partners\loading.tsx" -Raw
if ($check_loading_page -match [regex]::Escape("PartnersLoading")) { Write-Host "OK: src\app\admin\(dashboard)\partners\loading.tsx contains expected content" -ForegroundColor Green } else { Write-Host "PROBLEM: src\app\admin\(dashboard)\partners\loading.tsx missing expected content" -ForegroundColor Red }
if ($check_loading_page -match "[\u00C0-\u00FF]{2,}") { Write-Host "WARNING: src\app\admin\(dashboard)\partners\loading.tsx may contain garbled encoding" -ForegroundColor Yellow }
$check_admin_shell = Get-Content -LiteralPath "src\components\admin\AdminShell.tsx" -Raw
if ($check_admin_shell -match [regex]::Escape("label: `"Partners`"")) { Write-Host "OK: src\components\admin\AdminShell.tsx contains expected content" -ForegroundColor Green } else { Write-Host "PROBLEM: src\components\admin\AdminShell.tsx missing expected content" -ForegroundColor Red }
if ($check_admin_shell -match "[\u00C0-\u00FF]{2,}") { Write-Host "WARNING: src\components\admin\AdminShell.tsx may contain garbled encoding" -ForegroundColor Yellow }
...
Write-Host ""
Write-Host "Then visit /admin/partners to confirm no garbled characters appear." -ForegroundColor Cyan
