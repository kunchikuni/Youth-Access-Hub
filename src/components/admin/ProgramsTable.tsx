 ...
 * ProgramsTable
 *
 * Interactive table for the /admin/programs page.
 * Handles:
 *  - Status badge display (open / closed / coming-soon)
 *  - Featured badge
 *  - Toggle publish/unpublish in-place via Supabase
 *  - Delete with confirmation
 *  - Link to edit page
 *
 * @module components/admin/ProgramsTable
 */
...
export default function ProgramsTable({ programs: initial }: ProgramsTableProps) {
    ...
    <div className="tbl-wrap">
      ...
      {deleteSlug && (
        <div className="tbl-modal-overlay" role="dialog" aria-modal="true" aria-label="Confirm delete">
          <div className="tbl-modal">
            <h3 className="tbl-modal-title">Delete program?</h3>
            <p className="tbl-modal-body">
              This will permanently delete{" "}
              <strong>{programs.find((p) => p.slug === deleteSlug)?.title}</strong>.
        ...
        <table className="tbl">
          <thead>
            <tr>
              ...
              <th className="tbl-th">Start date</th>
              <th className="tbl-th tbl-th--actions">Actions</th>
            </tr>
          </thead>
          <tbody>
            {programs.map((program) => (
              <tr key={program.slug} className="tbl-row">
                {/* Title */}
                <td className="tbl-td">
                  <span className="tbl-title">{program.title}</span>
                  <span className="tbl-slug">{program.slug}</span>
                </td>

                {/* Category */}
                <td className="tbl-td">
                  <span className="tbl-category">{program.category}</span>
                </td>
                ...
                <td className="tbl-td">
                  {program.startDate ? (
                    <span className="tbl-date">
                      {new Date(program.startDate).toLocaleDateString("en-ZW", {
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                      })}
                ...
                <td className="tbl-td tbl-td--actions">
                  ...
                  {/* Toggle status */}
                  <button
                    className={`tbl-btn ${program.status === "open" ? "tbl-btn--warning" : "tbl-btn--success"}`}
                    onClick={() => handleToggleStatus(program)}
                    disabled={loadingSlug === program.slug}
                    aria-label={program.status === "open" ? `Close ${program.title}` : `Open ${program.title}`}
                  >
                    {loadingSlug === program.slug ? (
                      "…"
                    ) : program.status === "open" ? (
                      <>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
      ...
      <style>{`
        ...
        .tbl-btn--ghost {
          ...
          color: var(--yah-navy);
          border-color: var(--yah-light-gray);
        }

        .tbl-btn--ghost:hover:not(:disabled) {
          background: var(--yah-off-white);
        ...
        .tbl-btn--warning:hover:not(:disabled) {
          background: rgba(217,119,6,0.15);
        }

        .tbl-btn--danger-ghost {
          background: transparent;
          color: #dc2626;
          border-color: transparent;
        }

        .tbl-btn--danger-ghost:hover:not(:disabled) {
          background: rgba(220,38,38,0.06);
        ...
        .tbl-td--actions .tbl-btn + .tbl-btn {
