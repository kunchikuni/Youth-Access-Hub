 ...
 * Toggle status and delete now call server actions in
 * src/actions/programs.ts — no Supabase client DB calls
...
import { toggleProgramStatus, deleteProgram } from "@/actions/programs";
...
export default function ProgramsTable({ programs: initial }: ProgramsTableProps) {
  ...
  async function handleToggleStatus(program: Program) {
    ...
    const result = await toggleProgramStatus(program.slug, program.status);
    ...
    if (!result.success || !result.newStatus) {
      ...
      setPrograms((prev) =>
          prev.map((p) =>
              p.slug === program.slug ? { ...p, status: result.newStatus! } : p
          )
      );
    }
    setLoadingSlug(null);
