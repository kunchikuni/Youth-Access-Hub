 ...
 * src/actions/opportunities.ts — no Supabase client DB calls
...
import { toggleOpportunityStatus, deleteOpportunity } from "@/actions/opportunities";
...
export default function OpportunitiesTable({ opportunities: initial }: OpportunitiesTableProps) {
  ...
  async function handleToggleStatus(opp: Opportunity) {
    ...
    const result = await toggleOpportunityStatus(opp.slug, opp.status);
    ...
    if (!result.success || !result.newStatus) {
      ...
      setOpportunities((prev) =>
          prev.map((o) =>
              o.slug === opp.slug ? { ...o, status: result.newStatus! } : o
          )
      );
    }
    setLoadingSlug(null);
  ...
  async function handleDelete(slug: string) {
    ...
    const result = await deleteOpportunity(slug);
          ...
          <table className="tbl">
            ...
            {opportunities.map((opp) => (
                <tr key={opp.slug} className="tbl-row">
                  ...
                  <td className="tbl-td"><span className="tbl-category">{opp.category}</span></td>
                  <td className="tbl-td"><span className="tbl-provider">{opp.provider}</span></td>
                  <td className="tbl-td"><StatusBadge status={opp.status} /></td>
                  <td className="tbl-td">
                    {opp.featured ? (
                        <span className="tbl-badge" style={{ color: "var(--yah-sky)", background: "rgba(74,159,212,0.1)" }}>Featured</span>
                    ) : <span className="tbl-muted">—</span>}
                  </td>
                  <td className="tbl-td">
                    {opp.deadline ? (
                        <span className="tbl-date">
                      {new Date(opp.deadline).toLocaleDateString("en-ZW", { day: "numeric", month: "short", year: "numeric" })}
                    </span>
                    ) : <span className="tbl-muted">—</span>}
                  </td>
                  <td className="tbl-td tbl-td--actions">
                    <Link href={`/admin/opportunities/${opp.slug}`} className="tbl-btn tbl-btn--ghost" aria-label={`Edit ${opp.title}`}>
                    ...
                    <button className="tbl-btn tbl-btn--danger-ghost" onClick={() => setDeleteSlug(opp.slug)} disabled={loadingSlug === opp.slug} aria-label={`Delete ${opp.title}`}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
