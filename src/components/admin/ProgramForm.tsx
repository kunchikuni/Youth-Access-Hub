 ...
 *
 * All DB writes go through server actions in src/actions/programs.ts.
...
export default function ProgramForm({ program }: ProgramFormProps) {
  ...
  // Image — client uploads to Storage, passes URL to server action
  const [imageFile,      setImageFile]      = useState<File | null>(null);
  const [imagePreview,   setImagePreview]   = useState<string | null>(program?.coverImageUrl ?? null);
  ...
  const removeMentor = (i: number) =>
      setMentors((p) => p.filter((_, idx) => idx !== i));
  ...
  // Image upload to Supabase Storage (client-side, Option A)
  ...
  // Submit — image uploads client-side, then passes URL to server action
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Step 1: upload image if a new file was selected (client → Storage)
    ...
    // Step 2: call server action (browser → server → DB + revalidatePath)
    const payload = {
      slug:         slug.trim(),
      title:        title.trim(),
    ...
    const result = isEdit
        ? await updateProgram(program!.slug, payload)
